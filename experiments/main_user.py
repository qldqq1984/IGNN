...
from configs.user_configs import user_config_args, config_update
...
if __name__ == "__main__":
    ...
    args = user_config_args()  
    if args.toml is not None:
        tomls = "\n".join(args.toml)     
        new_config = toml.loads(tomls)          
    config = toml.load( '{:s}configs/user_config.toml'.format(args.DIR_ROOT) )
    ...
    x_ex_mean, x_ex_std = standardized_x_ex(device, DataLoader(train_dataset, shuffle = False, batch_size= args.num_train) )
    ...
    train_loader = DataLoader(train_dataset, shuffle=False, batch_size= args.num_train) 
    # Load the validation cohort for evaluation
    test_loader = DataLoader(test_dataset, shuffle=False, batch_size= args.num_test)            
    ...
    if args.MODEL_STATE == "Train":
        ...
        for epoch in range(1, args.epochs):
            ...
            trainRT_loss = experiment_info(device,train_dataset,  model, data_loader = trainRT_loader, TRAIN_RT = True, TRAIN_DATA = True,  cutoff_init = 0 ); # training batch  (n=128)    
