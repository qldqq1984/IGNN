...
*	The methods for adaptive training and validation of IGNN(IGNNE) model on raw data or user-defined data.
*	Reproduction of experimental results and displayable items demonstrated in the paper.    
...
***Note:*** Please ensure that the `Source Data` directory and `Source_Data_analysis` directory are in the same parent folder. Meanwhile, we recommend implementing these analytical programs in Rstudio to avoid manually setting the relevant working directory. Analysis programs based on R code usually depend on some specific R packages for specialized functions. When these programs are run for the first time, they will automatically download and install the dependent R packages. If the installation fails, please check the network connection status and the availability of the data sources for these R packages. Manual downloading and installation of some R packages is also optional.
# License
